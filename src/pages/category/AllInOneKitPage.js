...
const AllInOneKitPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for allinonekit category
        const allInOneKitProducts = items.filter(product => 
          hasCategory(product, "allinonekit")
        );
        
        setProducts(allInOneKitProducts);
        setSortedProducts(allInOneKitProducts);
        setCurrentData(allInOneKitProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO 
        titleTemplate="All In One Mobile Kit - Alharam"
        title="All In One Mobile Kit - Complete Mobile Protection Package"
        description="Shop all-in-one mobile kits at Alharam. Complete protection package including tempered glass, mobile cover, and accessories. Best value combo deals for all smartphone brands. Fast delivery across Pakistan." 
        keywords="all in one kit, mobile kit, phone protection kit, mobile combo, glass and cover combo, mobile protection package, smartphone kit, Pakistan, Alharam"
      />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "All In One Kit", path: process.env.PUBLIC_URL + "/allinonekit" },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
...
export default AllInOneKitPage;
