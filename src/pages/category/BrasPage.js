...
const BrasPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for bras category
        const brasProducts = items.filter(product => product.category && product.category.includes("bras"));
        setProducts(brasProducts);
        setSortedProducts(brasProducts);
        setCurrentData(brasProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO titleTemplate="Bras - Lanina" description="Shop premium bras at Lanina. Supportive, stylish, and comfortable bras for every shape and need." />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "Bras", path: process.env.PUBLIC_URL + pathname },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
