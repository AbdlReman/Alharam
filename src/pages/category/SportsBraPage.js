...
const SportsBraPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for sports bra category
        const sportsBraProducts = items.filter(product => 
          product.category && product.category.some(cat => 
            cat.toLowerCase().includes("sports bra") || 
            cat.toLowerCase().includes("sportsbra") ||
            (cat.toLowerCase().includes("bra") && cat.toLowerCase().includes("sports"))
          )
        );
        setProducts(sportsBraProducts);
        setSortedProducts(sportsBraProducts);
        setCurrentData(sportsBraProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO titleTemplate="Sports Bra - Lanina" description="Shop high-quality sports bras at Lanina. Supportive, comfortable, and perfect for active lifestyles." />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "Bras", path: process.env.PUBLIC_URL + "/bras" },
            { label: "Sports Bra", path: process.env.PUBLIC_URL + pathname },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
