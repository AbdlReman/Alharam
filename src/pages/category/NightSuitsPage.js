...
const NightSuitsPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for night suits category
        const nightSuitsProducts = items.filter(product => 
          product.category && product.category.some(cat => 
            cat.toLowerCase().includes("night suit") || 
            cat.toLowerCase().includes("nightsuit") ||
            cat.toLowerCase().includes("pajama") ||
            cat.toLowerCase().includes("pyjama")
          )
        );
        setProducts(nightSuitsProducts);
        setSortedProducts(nightSuitsProducts);
        setCurrentData(nightSuitsProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO titleTemplate="Night Suits - Lanina" description="Shop comfortable night suits at Lanina. Perfect pajama sets for a cozy night's sleep." />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "Nightwear", path: process.env.PUBLIC_URL + "/nightwear" },
            { label: "Night Suits", path: process.env.PUBLIC_URL + pathname },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
