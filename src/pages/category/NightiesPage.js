...
const NightiesPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for nighties category
        const nightiesProducts = items.filter(product => 
          product.category && product.category.some(cat => 
            cat.toLowerCase().includes("nighties") || 
            cat.toLowerCase().includes("nightie") ||
            cat.toLowerCase().includes("nightgown")
          )
        );
        setProducts(nightiesProducts);
        setSortedProducts(nightiesProducts);
        setCurrentData(nightiesProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO titleTemplate="Nighties - Lanina" description="Shop elegant nighties at Lanina. Comfortable and stylish sleepwear for a good night's rest." />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "Nightwear", path: process.env.PUBLIC_URL + "/nightwear" },
            { label: "Nighties", path: process.env.PUBLIC_URL + pathname },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
