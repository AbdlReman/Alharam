...
const StockingsPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for stockings category
        const stockingsProducts = items.filter(product => 
          product.category && product.category.some(cat => 
            cat.toLowerCase().includes("stockings") || 
            cat.toLowerCase().includes("hosiery") ||
            cat.toLowerCase().includes("pantyhose") ||
            cat.toLowerCase().includes("tights")
          )
        );
        setProducts(stockingsProducts);
        setSortedProducts(stockingsProducts);
        setCurrentData(stockingsProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO titleTemplate="Stockings - Lanina" description="Shop premium stockings and hosiery at Lanina. Elegant, comfortable, and stylish legwear for every occasion." />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "Stockings", path: process.env.PUBLIC_URL + pathname },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
