...
const TopsPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for tops category
        const topsProducts = items.filter(product => 
          product.category && product.category.some(cat => 
            cat.toLowerCase().includes("tops") || 
            cat.toLowerCase().includes("blouses") ||
            cat.toLowerCase().includes("shirts") ||
            cat.toLowerCase().includes("camisoles") ||
            cat.toLowerCase().includes("tanks")
          )
        );
        setProducts(topsProducts);
        setSortedProducts(topsProducts);
        setCurrentData(topsProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO titleTemplate="Tops - Lanina" description="Shop stylish tops and blouses at Lanina. Comfortable, fashionable, and perfect for any occasion." />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "Tops", path: process.env.PUBLIC_URL + pathname },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
