...
const PantiesPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for panties category
        const pantiesProducts = items.filter(product => product.category && product.category.includes("panties"));
        setProducts(pantiesProducts);
        setSortedProducts(pantiesProducts);
        setCurrentData(pantiesProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO titleTemplate="Panties - Lanina" description="Shop soft, breathable, and beautiful panties at Lanina. Premium quality panties for everyday comfort." />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "Panties", path: process.env.PUBLIC_URL + pathname },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
