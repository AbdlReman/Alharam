...
const OnlyCasePage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for onlycase category
        const onlyCaseProducts = items.filter(product => 
          hasCategory(product, "onlycase")
        );
        
        setProducts(onlyCaseProducts);
        setSortedProducts(onlyCaseProducts);
        setCurrentData(onlyCaseProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO 
        titleTemplate="Mobile Covers & Cases - Alharam"
        title="Mobile Covers & Cases - Stylish Protection for Your Phone"
        description="Shop premium mobile covers and cases at Alharam. Wide range of stylish, durable, and protective phone cases for all brands. Fast delivery across Pakistan." 
        keywords="mobile covers, phone cases, mobile case, phone cover, back cover, protective case, smartphone cases, Pakistan, Alharam"
      />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "Mobile Covers", path: process.env.PUBLIC_URL + "/onlycase" },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
