...
const NursingBraPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for nursing bra category
        const nursingBraProducts = items.filter(product => 
          product.category && product.category.some(cat => 
            cat.toLowerCase().includes("nursing bra") || 
            cat.toLowerCase().includes("nursingbra") ||
            (cat.toLowerCase().includes("bra") && cat.toLowerCase().includes("nursing"))
          )
        );
        setProducts(nursingBraProducts);
        setSortedProducts(nursingBraProducts);
        setCurrentData(nursingBraProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO titleTemplate="Nursing Bra - Lanina" description="Shop comfortable nursing bras at Lanina. Designed for comfort and convenience during breastfeeding." />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "Bras", path: process.env.PUBLIC_URL + "/bras" },
            { label: "Nursing Bra", path: process.env.PUBLIC_URL + pathname },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
