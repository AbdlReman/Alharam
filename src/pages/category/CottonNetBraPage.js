...
const CottonNetBraPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for cotton & net bra category
        const cottonNetBraProducts = items.filter(product => 
          product.category && product.category.some(cat => 
            cat.toLowerCase().includes("cotton") && cat.toLowerCase().includes("net") ||
            cat.toLowerCase().includes("cotton & net") ||
            cat.toLowerCase().includes("cotton net bra")
          )
        );
        setProducts(cottonNetBraProducts);
        setSortedProducts(cottonNetBraProducts);
        setCurrentData(cottonNetBraProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO titleTemplate="Cotton & Net Bra - Lanina" description="Shop comfortable cotton and net bras at Lanina. Breathable and stylish for everyday wear." />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "Bras", path: process.env.PUBLIC_URL + "/bras" },
            { label: "Cotton & Net Bra", path: process.env.PUBLIC_URL + pathname },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
...
export default CottonNetBraPage;
