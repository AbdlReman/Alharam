...
const BoldWearPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for bold wear category
        const boldWearProducts = items.filter(product => 
          product.category && product.category.some(cat => 
            cat.toLowerCase().includes("bold wear") || 
            cat.toLowerCase().includes("boldwear") ||
            cat.toLowerCase().includes("bold")
          )
        );
        setProducts(boldWearProducts);
        setSortedProducts(boldWearProducts);
        setCurrentData(boldWearProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO titleTemplate="Bold Wear - Lanina" description="Shop bold and stylish nightwear at Lanina. Make a statement with our bold wear collection." />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "Nightwear", path: process.env.PUBLIC_URL + "/nightwear" },
            { label: "Bold Wear", path: process.env.PUBLIC_URL + pathname },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
