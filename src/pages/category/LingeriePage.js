...
const LingeriePage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for lingerie category
        const lingerieProducts = items.filter(product => product.category && product.category.includes("lingerie"));
        setProducts(lingerieProducts);
        setSortedProducts(lingerieProducts);
        setCurrentData(lingerieProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO titleTemplate="Lingerie - Lanina" description="Shop elegant and alluring lingerie at Lanina. Premium quality lingerie to make every woman feel special and beautiful." />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "Lingerie", path: process.env.PUBLIC_URL + pathname },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
