...
const ShapewearPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for shapewear category
        const shapewearProducts = items.filter(product => product.category && product.category.includes("shapewear"));
        setProducts(shapewearProducts);
        setSortedProducts(shapewearProducts);
        setCurrentData(shapewearProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO titleTemplate="Shapewear - Lanina" description="Shop comfortable shapewear at Lanina. Premium quality shapewear for a smooth, confident silhouette." />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "Shapewear", path: process.env.PUBLIC_URL + pathname },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
