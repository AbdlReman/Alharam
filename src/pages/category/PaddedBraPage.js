...
const PaddedBraPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for padded bra category
        const paddedBraProducts = items.filter(product => 
          product.category && product.category.some(cat => 
            cat.toLowerCase().includes("padded bra") || 
            cat.toLowerCase().includes("paddedbra") ||
            (cat.toLowerCase().includes("bra") && cat.toLowerCase().includes("padded"))
          )
        );
        setProducts(paddedBraProducts);
        setSortedProducts(paddedBraProducts);
        setCurrentData(paddedBraProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO titleTemplate="Padded Bra - Lanina" description="Shop comfortable padded bras at Lanina. Enhanced shape and support for everyday confidence." />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "Bras", path: process.env.PUBLIC_URL + "/bras" },
            { label: "Padded Bra", path: process.env.PUBLIC_URL + pathname },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
