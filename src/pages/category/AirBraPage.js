...
const AirBraPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for air bra category
        const airBraProducts = items.filter(product => 
          product.category && product.category.some(cat => 
            cat.toLowerCase().includes("air bra") || 
            cat.toLowerCase().includes("airbra") ||
            (cat.toLowerCase().includes("bra") && cat.toLowerCase().includes("air"))
          )
        );
        setProducts(airBraProducts);
        setSortedProducts(airBraProducts);
        setCurrentData(airBraProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO titleTemplate="Air Bra - Lanina" description="Shop comfortable air bras at Lanina. Lightweight, breathable, and perfect for everyday wear." />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "Bras", path: process.env.PUBLIC_URL + "/bras" },
            { label: "Air Bra", path: process.env.PUBLIC_URL + pathname },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
