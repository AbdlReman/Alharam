...
const OnlyGlassPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for onlyglass category
        const onlyGlassProducts = items.filter(product => 
          hasCategory(product, "onlyglass")
        );
        
        setProducts(onlyGlassProducts);
        setSortedProducts(onlyGlassProducts);
        setCurrentData(onlyGlassProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO 
        titleTemplate="Mobile Glass Protectors - Alharam"
        title="Mobile Glass Protectors - Premium Tempered Glass & Screen Guards"
        description="Shop premium mobile glass protectors and tempered glass at Alharam. Perfect screen protection for all smartphone brands. 9H hardness, anti-scratch, and crystal clear display. Fast delivery across Pakistan." 
        keywords="mobile glass, tempered glass, screen protector, screen guard, mobile screen protection, 9H glass, anti-scratch, smartphone glass, Pakistan, Alharam"
      />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "Mobile Glass", path: process.env.PUBLIC_URL + "/onlyglass" },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
