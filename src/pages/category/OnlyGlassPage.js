...
const OnlyGlassPage = () => {
  ...
  useEffect(() => {
    ...
    filtered = filtered.filter((product) =>
      product.category && (Array.isArray(product.category)
        ? product.category.includes("onlyglass")
        : product.category === "onlyglass")
    );
  ...
  return (
    <Fragment>
      <SEO
        titleTemplate="OnlyGlass - Alharam"
        description="Shop OnlyGlass products at Alharam. Premium tempered glass and screen protection."
      />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "OnlyGlass", path: process.env.PUBLIC_URL + "/onlyglass" },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
                ...
                <div className="col-lg-3 order-2 order-lg-1">
                  <ShopSidebar
                    products={[...products].filter(p => {
                      // filter by page category and search only (ignore selectedColor)
                      const inCategory = p.category && (Array.isArray(p.category) ? p.category.includes("onlyglass") : p.category === "onlyglass");
                  ...
                  <div className="category-header mb-4">
                    <div className="row">
                      <div className="col-12">
                        <h1 className="category-title">OnlyGlass</h1>
                        <p className="category-description">
                          Premium screen protectors and tempered glass for your devices.
                        </p>
                      </div>
                    </div>
                  </div>
