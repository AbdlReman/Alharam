...
const StraplessBraPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for strapless bra category
        const straplessBraProducts = items.filter(product => 
          product.category && product.category.some(cat => 
            cat.toLowerCase().includes("strapless bra") || 
            cat.toLowerCase().includes("straplessbra") ||
            (cat.toLowerCase().includes("bra") && cat.toLowerCase().includes("strapless"))
          )
        );
        setProducts(straplessBraProducts);
        setSortedProducts(straplessBraProducts);
        setCurrentData(straplessBraProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO titleTemplate="Strapless Bra - Lanina" description="Shop stylish strapless bras at Lanina. Perfect for off-shoulder and strapless outfits." />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "Bras", path: process.env.PUBLIC_URL + "/bras" },
            { label: "Strapless Bra", path: process.env.PUBLIC_URL + pathname },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
...
export default StraplessBraPage;
