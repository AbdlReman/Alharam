...
const UndergarmentsPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for undergarments category
        const undergarmentsProducts = items.filter(product => 
          product.category && product.category.some(cat => 
            cat.toLowerCase().includes("undergarments") || 
            cat.toLowerCase().includes("underwear") ||
            cat.toLowerCase().includes("intimate")
          )
        );
        setProducts(undergarmentsProducts);
        setSortedProducts(undergarmentsProducts);
        setCurrentData(undergarmentsProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO titleTemplate="Undergarments - Lanina" description="Shop premium undergarments at Lanina. Comfort, quality, and style for intimate wear." />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "Undergarments", path: process.env.PUBLIC_URL + pathname },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
...
export default UndergarmentsPage;
