  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const electronicsProducts = items.filter(product => 
          hasCategory(product, "electronics")
        );
        
        setProducts(electronicsProducts);
        setSortedProducts(electronicsProducts);
        setCurrentData(electronicsProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO 
        ...
        title="Electronics - AC, Freezers & Home Appliances"
        description="Shop quality electronics at Alharam. Wide range of AC units, freezers, and home appliances. Premium quality with warranty and fast delivery across Pakistan." 
        keywords="electronics, AC, freezer, home appliances, air conditioner, refrigerator, Pakistan, Alharam"
      />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "Electronics", path: process.env.PUBLIC_URL + "/electronics" },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
